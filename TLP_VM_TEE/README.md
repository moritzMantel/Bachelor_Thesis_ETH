...
> **tl;dr** An HTTP server that enforces TLP-based rate limiting. Clients must solve a time-lock puzzle before accessing the service endpoint. Intended to run inside a confidential VM (e.g. AMD SEV-SNP) so the host cannot tamper with the puzzle parameters or skip verification.

---

## Overview

The server exposes two endpoints and uses a Time-Lock Puzzle (TLP) as a rate-limiting primitive: before a client can call the actual service, it must solve a puzzle that takes a bounded amount of sequential computation. Because the puzzle parameters are generated and verified server-side inside the TEE, a malicious host cannot hand out easy puzzles or bypass verification.
...
- `request_handler()` dispatches incoming requests to `/request_TLP` or `/solve`, parses query parameters, and builds the response

### `src/rate_limiting.c`

Core TLP logic: RSA group setup, puzzle generation, and verification.
